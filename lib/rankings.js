  ...
  sanitized () {
    return {
      ...
      scores: this.scores
        ...
        .map(score => this.hideNegatives ? Math.max(score.score, 0) : score.score),
      highest: this.orderedScores.length ? this.orderedScores[0].score : undefined
    }
  }
}
...
Rank.compare = function (rank1, rank2) {
  if (rank1.orderedScores.length === 0 && rank2.orderedScores.length === 0) {
    return rank2.teamNumber - rank1.teamNumber
  }
