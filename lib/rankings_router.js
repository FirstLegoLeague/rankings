...
function getRankings (logger) {
  return Promise.all([Stage.get(), Scores.get(), Teams.get()])
    .then(responses => {
      const stage = responses[0].toLowerCase()
      const scores = responses[1].filter(score => score.public && score.stage.toLowerCase() === stage)
      const teams = responses[2]
      logger.info(`Calculating rankings of ${teams.length} teams with ${scores.length} scores `)
      return rankings(scores, teams)
    })
}

router.use('/rankings.json', (req, res) => {
  getRankings(req.logger).then(ranks => {
    res.json(ranks)
  }).catch(err => {
