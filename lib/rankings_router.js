...
function getRankings (req) {
  return Promise.all([Stage.get(), Scores.get(), Teams.get(), Configuration.get('hideNegatives')])
    .then(([stageResponse, scoresResponse, teamsResponse, hideNegativesConfiguration]) => {
      const stage = stageResponse.toLowerCase()
      const scores = scoresResponse.filter(score => score.public && score.stage.toLowerCase() === stage)
